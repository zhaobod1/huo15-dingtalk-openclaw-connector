 ...
 */
export async function getUnionId(staffId, config, log) {
    // 检查缓存
 ...
 * 停止定时清理机制
 */
export function stopMessageCleanup() {
    if (cleanupTimer) {
 ...
 */
export function checkAndMarkDingtalkMessage(accountId, protocolMessageId, businessMsgId) {
    ...
    if (scopedProtocolId)
        markMessageProcessed(scopedProtocolId);
    if (scopedBusinessId)
        markMessageProcessed(scopedBusinessId);
    return false;
}
// ============ 配置工具 ============
/**
 * 获取钉钉配置
 */
export function getDingtalkConfig(cfg) {
    return cfg?.channels?.['dingtalk-connector'] || {};
}
/**
 * 检查是否已配置
 */
export function isDingtalkConfigured(cfg) {
 ...
 */
export async function addEmotionReply(config, data, log) {
    if (!data.msgId || !data.conversationId)
        return;
    try {
 ...
 * 撤回用户消息上的 🤔思考中 表情
 */
export async function recallEmotionReply(config, data, log) {
    if (!data.msgId || !data.conversationId)
        return;
    try {
        const token = await getAccessToken(config);
