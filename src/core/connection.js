...
import { checkAndMarkDingtalkMessage, } from "../utils/utils-legacy.js";
// ============ 连接配置 ============
/** 心跳间隔（毫秒） */
...
// ============ 监控账号 ============
export async function monitorSingleAccount(opts) {
    ...
    if (!account.clientId || !account.clientSecret) {
        throw new Error(`[DingTalk][${accountId}] Missing credentials: ` +
    ...
    if (clientIdStr.length < 10 || clientSecretStr.length < 10) {
        throw new Error(`[DingTalk][${accountId}] Invalid credentials format: ` +
        ...
        keepAliveTimer = setInterval(async () => {
            ...
            try {
                ...
                if (elapsed > TIMEOUT_THRESHOLD) {
                    logger.info(`⚠️ 超时检测：已 ${Math.round(elapsed / 1000)} 秒未确认 socket 可用，触发重连...`);
                    await doReconnect();
                    return;
                }
                // 【心跳检测】检查 socket 状态
                ...
                logger.debug(`🔍 心跳检测：socket 状态=${socketState}, elapsed=${Math.round(elapsed / 1000)}s, 连接已建立=${Math.round(timeSinceConnection / 1000)}s`);
                // 给新建立的连接 15 秒宽限期，避免在连接建立初期就触发重连
                if (socketState !== 1) {
                    if (timeSinceConnection < 15_000) {
                        logger.debug(`⏳ 连接建立中（已 ${Math.round(timeSinceConnection / 1000)}s），跳过状态检查`);
                        return;
                    }
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const statsInterval = setInterval(() => {
            ...
            logger.info(`统计：收到=${receivedCount}, 处理=${processedCount}, ` +
        ...
        client.registerCallbackListener(TOPIC_ROBOT, async (res) => {
            ...
            try {
                ...
                try {
                    ...
                    throw new Error(`Invalid JSON response from DingTalk API. ` +
                        `Error: ${parseError instanceof Error ? parseError.message : String(parseError)}. ` +
        ...
        try {
            ...
            if (error.response?.status === 400 || error.message?.includes("status code 400") || error.message?.includes("400")) {
                throw new Error(`[DingTalk][${accountId}] Bad Request (400):\n` +
            ...
            if (error.response?.status === 401 || error.message?.includes("401")) {
                throw new Error(`[DingTalk][${accountId}] Authentication failed (401 Unauthorized):\n` +
            ...
            // 处理其他连接错误
            throw new Error(`[DingTalk][${accountId}] Failed to connect to DingTalk Stream: ${error.message}`);
        }
...
export function resolveReactionSyntheticEvent(event) {
    void event;
    return null;
}
