import { buildSessionContext, getAccessToken, getOapiAccessToken, DINGTALK_API, addEmotionReply, recallEmotionReply, } from "../utils/utils-legacy.js";
...
import { sendProactive } from "../services/messaging/index.js";
import { createAICardForTarget, streamAICard } from "../services/messaging/card.js";
...
function extractQuotedMsgText(container, maxDepth) {
    if (maxDepth <= 0 || !container)
        return null;
    if (!container.isReplyMsg)
        return null;
    ...
    switch (msgType) {
        ...
        case 'richText': {
            ...
            const textParts = richList
                .filter((item) => item.text && item.msgType !== 'skill' && !item.skillData)
                .map((item) => item.text);
...
function extractRichTextMediaAttachments(data, content) {
    ...
    const fileNames = [];
    // 兼容新结构 content.richText 和旧结构 richText.richTextList
    const richList = content?.richText ||
...
export function extractMessageContent(data) {
    ...
    switch (msgtype) {
        ...
        case 'actionCard': {
            ...
            // 提取操作链接列表
            const actionUrlItems = content?.actionUrlItemList || [];
            const actionUrls = actionUrlItems
                .map((item) => item.actionUrl?.trim())
                .filter((url) => !!url);
            const sections = [];
            if (title)
                sections.push(title);
            ...
            if (actionUrls.length > 0) {
                const linkSection = actionUrls.length === 1
                    ? `操作链接：${actionUrls[0]}`
                    : `操作链接：\n- ${actionUrls.join('\n- ')}`;
        ...
        case 'interactiveCard': {
            ...
            const interactiveCardUrl = (content?.biz_custom_action_url || data?.biz_custom_action_url || '').trim() || undefined;
...
// ============ 图片下载 ============
export async function downloadImageToFile(downloadUrl, agentWorkspaceDir, log) {
    try {
...
export async function downloadMediaByCode(downloadCode, config, agentWorkspaceDir, log) {
    try {
        const token = await getAccessToken(config);
        log?.info?.(`通过 downloadCode 下载媒体: ${downloadCode.slice(0, 30)}...`);
        const resp = await dingtalkHttp.post(`${DINGTALK_API}/v1.0/robot/messageFiles/download`, { downloadCode, robotCode: config.clientId }, {
...
export async function getFileDownloadUrl(downloadCode, fileName, config, log) {
    try {
        const token = await getAccessToken(config);
 ...
 * 下载文件到本地
 */
export async function downloadFileToLocal(downloadUrl, fileName, agentWorkspaceDir, log) {
    try {
 ...
 * 内部消息处理函数（实际执行消息处理逻辑）
 */
export async function handleDingTalkMessageInternal(params) {
    ...
    if (!content.text && content.imageUrls.length === 0 && content.downloadCodes.length === 0)
        return;
    ...
    if (!userContent && imageLocalPaths.length === 0)
        return;
    // ===== 贴处理中表情 =====
    ...
    try {
        ...
        if (asyncMode) {
            try {
                ...
                if (oapiToken) {
                    ...
                    finalText = await processVideoMarkers(finalText, '', config, oapiToken, log, true, // ✅ 使用主动 API 模式
                    mediaTarget);
                    finalText = await processAudioMarkers(finalText, '', config, oapiToken, log, true, // ✅ 使用主动 API 模式
                    mediaTarget);
                    finalText = await processFileMarkers(finalText, '', config, oapiToken, log, true, // ✅ 使用主动 API 模式
                    mediaTarget);
        ...
        try {
            ...
            if (!isDirect)
                body.at = { atUserIds: [senderId], isAtAll: false };
