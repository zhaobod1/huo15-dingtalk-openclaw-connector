...
import { DINGTALK_OAPI } from '../utils/index.js';
...
export async function uploadMediaToDingTalk(filePath, mediaType, oapiToken, maxSize = 20 * 1024 * 1024, log) {
    try {
        ...
        const resp = await dingtalkOapiHttp.post(`${DINGTALK_OAPI}/media/upload?access_token=${oapiToken}&type=${mediaType === 'video' ? 'file' : mediaType}`, form, { headers: form.getHeaders(), timeout: 60_000 });
 ...
 */
export async function processLocalImages(content, oapiToken, log) {
    if (!oapiToken) {
...
export async function processAudioMarkers(content, sessionWebhook, config, oapiToken, log, useProactiveApi = false, target) {
    ...
    for (const match of matches) {
        try {
            const audioInfo = JSON.parse(match[1]);
 ...
 * 提取文件标记并发送文件消息
 */
export async function processFileMarkers(content, sessionWebhook, config, oapiToken, log, useProactiveApi = false, target) {
 ...
 * 发送视频消息（主动 API 模式）
 */
export async function sendVideoProactive(config, target, videoMediaId, picMediaId, metadata, log) {
    try {
        const token = await (await import('../utils/index.js')).getAccessToken(config);
 ...
 */
async function sendAudioMessage(config, sessionWebhook, fileName, mediaId, log, durationMs) {
    try {
        const token = await (await import('../utils/index.js')).getAccessToken(config);
        // 钉钉语音消息格式
 ...
 * 发送音频消息（主动 API 模式）
 */
export async function sendAudioProactive(config, target, fileName, mediaId, log, durationMs) {
    try {
        const token = await (await import('../utils/index.js')).getAccessToken(config);
        const { DINGTALK_API } = await import('../utils/index.js');
        // 钉钉普通消息 API 的音频消息格式
 ...
 */
async function sendFileMessage(config, sessionWebhook, fileInfo, mediaId, log) {
    try {
        const token = await (await import('../utils/index.js')).getAccessToken(config);
        const fileMessage = {
            msgtype: 'file',
 ...
 * 发送文件消息（主动 API 模式）
 */
export async function sendFileProactive(config, target, fileInfo, mediaId, log) {
    try {
        const token = await (await import('../utils/index.js')).getAccessToken(config);
        const { DINGTALK_API } = await import('../utils/index.js');
        // 钉钉普通消息 API 的文件消息格式
...
export async function processRawMediaPaths(content, config, oapiToken, log, target) {
